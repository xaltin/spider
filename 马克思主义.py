...
def get_pic():      #生成器，返回一个图片链接
    ...
    while True:
        ...
        pic = re.compile(r'''<a\s+?
                             class="directlink\s+?
                             largeimg"\s+?
                             href="(https://files.yande.re/.+?\.(?:jpg|jpeg|gif))"
                             >''',re.VERBOSE)
...
def download():     #下载图片
    global headers
    count = 1
    for pic_url in get_pic():         #get_pic(1)表示从第1页开始下载
...
if __name__ == '__main__':
    global page
    get_proxy()     #取得代理
    create_localhost()  #添加本地直接连接
    try:
        download()  #下载图片
    except KeyboardInterrupt as e:      #Ctrl+C
        print('中止下载，当前页面:%d' % page)
